...
void scheduleProgram4(process_t** processes, int numProcesses, int quantum, char *mem) {
    ...
    while (finished < numProcesses) {
        ...
        if (!isEmpty(&readyQueue)) {
            ...
            // If process was not yet referenced (change this for second-chance algorithm)
            ...
            if ((currentProcess->isAllocated == 0) || 
                ((currentProcess->pageAllocated < 4) && (currentProcess->pageAllocated < calculatePageNeeded(currentProcess)))) {
    ...
    printf("Turnaround time %d\nTime overhead %.2f %.2f\nMakespan %d\n", (int)ceil(totalTurnaround), maxOverhead, averageOverhead, makeSpan);
